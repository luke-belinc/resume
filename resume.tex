\documentclass[15pt]{article}
\usepackage{fontspec}
% \setmainfont{MonoLisa-Light}[
%     Path           = fonts/MonoLisa/,
%     Extension      = .otf,
%     BoldFont       = MonoLisa-Bold,
%     ItalicFont     = MonoLisa-LightItalic,
%     BoldItalicFont = MonoLisa-BoldItalic,
%     Ligatures      = Common,
%     Numbers        = SlashedZero,
%     RawFeature     = {+calt,+zero,+ss02,+ss03},
%     LetterSpace    = -10
% ]
% had to do this because i have so many texttt things everywhere
% \setmonofont{MonoLisa-Light}[
%     Path           = fonts/MonoLisa/,
%     Extension      = .otf,
%     BoldFont       = MonoLisa-Bold,
%     ItalicFont     = MonoLisa-LightItalic,
%     BoldItalicFont = MonoLisa-BoldItalic,
%     Ligatures      = Common,
%     Numbers        = SlashedZero,
%     RawFeature     = {+calt,+zero,+ss02,+ss03},
%     LetterSpace    = -7.5
% ]
% \newfontface\mlscript{MonoLisa-BoldItalic}[
%     Path = fonts/MonoLisa/,
%     Extension = .otf,
%     RawFeature = +calt;-liga;+zero;+ss02,
%     LetterSpace = -12,
%     BoldItalicFont = MonoLisa-BoldItalic,
%     BoldFont = MonoLisa-BoldItalic,
%     ItalicFont = MonoLisa-BoldItalic
% ]
\newfontface\mltitle{MonoLisa-Bold}[
    Path = fonts/MonoLisa/,
    Extension = .otf,
    RawFeature = +calt;-liga;+zero;+ss02,
    LetterSpace = -12,
    BoldItalicFont = MonoLisa-Bold,
    BoldFont = MonoLisa-Bold,
    ItalicFont = MonoLisa-Bold
]

\usepackage[left=.75cm, right=.55cm, top=.35cm, bottom=0cm]{geometry}
\usepackage[hidelinks]{hyperref}
\usepackage{fontawesome5}
\usepackage{tabularx, ragged2e, calc, booktabs, sectsty, anyfontsize, enumitem}

\newenvironment{itemize*}%
  {\begin{itemize}[leftmargin=*,label=\faChevronRight]%
    \setlength{\itemsep}{0pt}%
    \setlength{\parskip}{0pt}%
    \setlength{\parsep}{0pt}}%
  {\end{itemize}}

\pagenumbering{gobble}
\sectionfont{\fontsize{17}{15}}
\parindent=0pt
\title{Resume}
\author{Luke Belinc}
\date{September 12, 2022}

% fixed coloumn width and alignment
\newcolumntype{L}[1]{>{\raggedright\arraybackslash}p{#1}}
\newcolumntype{C}[1]{>{\centering\arraybackslash}p{#1}}
\newcolumntype{R}[1]{>{\raggedleft\arraybackslash}p{#1}}

% generate work experience
\newcommand\workexp[5]{
    \begin{minipage}[t]{0.165\textwidth}
            \raggedleft
                \textbf{\fontsize{14}{12}\selectfont #1}\\ 
                #2\\
                \small\texttt{#3}\textendash \texttt{#4}
    \end{minipage}\hspace*{5pt}%
    \begin{minipage}[t]{0.8\textwidth}
        \raggedright
        \fontsize{13}{13}\selectfont#5
    \end{minipage}
}

\newlength{\letterheight}
\newlength{\iconheight}
\newlength{\heightdif}
\newcommand\infolink[3]{%
    \settototalheight{\iconheight}{#1}%
    \settototalheight{\letterheight}{#2}%
    \setlength{\heightdif}{\letterheight-\iconheight}%
    \fontsize{13pt}{12pt}\raisebox{-0.25\heightdif}{#1\hspace*{1pt}}\ttfamily\href{#2}{#3}%
}

\begin{document}

\bgroup
\begin{center}
    \fontsize{45}{45}\mltitle Luke Belinc
\end{center}
\egroup

\bgroup
\begin{center}
    \begin{tabular}{L{0.2\textwidth} C{0.2\textwidth} C{0.2\textwidth} R{0.2\textwidth}}
        \infolink{\faPhoneSquare}{tel:7349695442}{(734)969-5442}                            &
        \infolink{\faLinkedinIn}{https://www.linkedin.com/in/luke-belinc/}{/in/luke-belinc} &
        \infolink{\faGithub*}{https://github.com/luke-belinc}{/luke-belinc}                 &
        \infolink{\faAt}{mailto:belinc@tuta.io}{belinc@tuta.io}
    \end{tabular}
\end{center}
\egroup
\hrule
\vspace*{-10pt}
\section*{Experience}
\vspace*{-5pt}\workexp{Software\\ Engineer}{Bloomerang}{10/2022}{Current}{%
    \begin{itemize*}
        \item Design and build Pepper, an ETL framework in Go for nonprofit CRM data conversions, featuring a custom DSL, 
              deduplication engine, statistical reporting, and schema-driven transformation pipeline
        \item Develop Bob, an automation utility that handles routine data conversions, reducing manual effort and error 
              rates for the Conversion Engineering team
        \item Execute customer data conversions directly, providing firsthand insight into team workflows and informing tooling improvements
        \item Collaborate with Conversion Data Analysts and Project Managers to implement data transformation requirements
        \item Received ``above and beyond'' in monthly performance reviews
    \end{itemize*}%
}
\vspace*{10pt}\\
\workexp{Associate\\ Software\\ Engineer}{Verint Systems}{02/2022}{08/2022}{%
\begin{itemize*}
    \item Built and maintained internal software tools for the Experience Management Product Support organization
    \item Automated workflows to create efficiency of scale while also reducing manual errors
    \item Built and deployed single page web applications via AWS EC2
    \item Tasked with providing all necessary CLI training to colleagues
    \item Utilized Agile software development approach with each project
    \item Acted as admin for 20+ mission critical PowerBI waterfall reports and data gateways
\end{itemize*}%
}
\vspace*{10pt}\\
\workexp{Systems\\ Programmer}{WSU\textendash Huang Lab}{03/2020}{10/2020}{%
    \begin{itemize*}
        \item Built and bedugged software extensions for the GEOS-Chem and WRF-Chem modeling systems
        \item Drew up comprehensive bug reports to identify pain points and provided solutions
        \item Authored scripts to interpret and generate maps from atmospheric data models
        \item Leveraged Bash, cron, and Slurm via WSU's HPC nodes to automate and test software, data collection, and data modeling
        \item Implemented GNU Make to build software and extensions
    \end{itemize*}%
}

\vspace*{-10pt}
\section*{Skill Set}
\vspace*{-5pt}\hspace*{10pt}\begin{minipage}{0.935\textwidth}
    \begin{description}
        \itemsep0em
        \raggedright
        \item[Languages] C, C\texttt{++}, Python, Go, C\texttt{\#}, Java, HTML/CSS, Bash
        \item[Frameworks\texttt{/}Libraries] POSIX, glibc, Angular, React, Express, Jasmine, pandas, Tkinter, NumPy, NLTK,
            Qt, .NET
        \item[Tools] Unix shells \texttt{(}zsh, sh, Bash\texttt{)}, GNU coreutils, Vim, Slurm, cron, PowerShell, Git, \LaTeX, Node.js,
            AWS \texttt{(}S3, EC2, Lambda\texttt{)}, Database Managament Systems \texttt{(}PostgreSQL, MySQL, SQLite\texttt{)}, Jira, Confluence, Notion
        \item[Methodologies] Agile, CI\texttt{/}CD, Behavior- and Test-driven development
    \end{description}
\end{minipage}

\vspace*{-10pt}
\section*{Projects\texttt{/}Hackathons}
\vspace*{-5pt}\hspace*{10pt}\begin{minipage}{0.93\textwidth}
    \begin{description}
        \itemsep0em
        \raggedright
        \item[AirSkyBoat \href{https://github.com/luke-belinc/AirSkyBoat}{\fontsize{9pt}{9pt}\faLink}] Continuous contribution to 
            development of open source server emulator for Final Fantasy XI.
        \item[DHack \href{https://github.com/kraudo/keepup}{\fontsize{9pt}{9pt}\faLink}] Designed health monitoring app using AWS 
            Lambda, JavaScript and React Native. Won 2nd place overall and 1st place for the FitBit challenge.
        \item[Chinese Character Dictionary \href{https://github.com/luke-belinc/chinese-char-dict}{\fontsize{9pt}{9pt}\faLink}] 
            Designed a simple character meaning and stroke order dictionary and thesaurus using Python, PostgreSQL, and Qt
        \item[Mock Shell \href{https://github.com/luke-belinc/c-fun}{\fontsize{9pt}{9pt}\faLink}] Mock POSIX-like shell built in C. Implemented libraries such as \texttt{stdlib.h}, \texttt{signal.h}, 
            and \texttt{unistd.h}.
    \end{description}
\end{minipage}

\vspace*{15pt}
\begin{minipage}[t]{0.245\textwidth}
    \section*{Education}
    \vspace*{-5pt}\raggedright
    \hspace*{5pt}\textbf{Wayne State University}\\
    \hspace*{5pt}B.S. in Computer Science \\
    \hspace*{5pt}\textit{Detroit, MI}\hspace*{5pt} \\
    \hspace*{5pt}\textit{\texttt{2016}\textendash \texttt{2021}}
\end{minipage}\hspace*{30pt}%
\begin{minipage}[t]{0.7\textwidth}%
    \vspace*{-14.5pt}\section*{References}
    \vspace*{-5pt}\begin{tabular*}{\textwidth}{L{0.5\textwidth} L{0.5\textwidth}}
        \textbf{Michael Bowen} & \textbf{Matt Crawford} \\
        Sr. Director, Getting Started & Director, Product Support \\
        \infolink{\faPhoneSquare}{tel:2488952682}{(248)895-2682} & \infolink{\faPhoneSquare}{tel:2485955635}{(248)595-5635} \\
        \infolink{\faLinkedinIn}{https://www.linkedin.com/in/michael-bowen-it/}{/in/michael-bowen-it} &
        \infolink{\faLinkedinIn}{https://www.linkedin.com/in/th3mattcrawford/}{/in/th3mattcrawford}
    \end{tabular*}
\end{minipage}

\end{document}
